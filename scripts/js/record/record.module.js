...
recordModule.factory('USERID', function ($resource) {
    ...
    if (onlineOrLocal=='local')
        ...
        return $resource('/api-optimise/wh/gettoken4c.php',{},{
        });
    }
});
...
recordModule.service('records', function (Record, Edit, USUBJID, $http, $q, USUBJIDList,
    ...
    var getURL = function(functionName) {
        ...
        if (functionName == 'Delete') {
            if (onlineOrLocal == 'local')
                api = 'http://www.optimise-ms.org/api-optimise/wh/patients.php';
            else
                api = '/api-optimise/wh/patients.php';
        }

        if (functionName == 'USUBJIDList') {
            if (onlineOrLocal == 'local')
                api = 'http://www.optimise-ms.org/api-optimise/actions.php?action=list&fieldName=USUBJID';
            else
                api = '/api-optimise/actions.php?action=list&fieldName=USUBJID';
        }

        if (functionName == 'NHSIDList') {
            if (onlineOrLocal == 'local')
                api = 'http://www.optimise-ms.org/api-optimise/actions.php?action=list&fieldName=NHS_USUBJID';
            else
                api = '/api-optimise/actions.php?action=list&fieldName=NHS_USUBJID';
        }
        ...
        if (functionName == 'NewSubject') {
            if (onlineOrLocal == 'local')
                api = 'http://www.optimise-ms.org/api-optimise/wh/patients.php?action=new';
            else
                api = '/api-optimise/wh/patients.php?action=new';
        }
